...
`.trim()
    conn.caklontong[id] = [
        await conn.sendButton(m.chat, caption, author, null, [['⫷ ʙᴀɴᴛᴜᴀɴ ⫸', `${usedPrefix}calo`]], m),
        json, poin,
        setTimeout(async () => {
            if (conn.caklontong[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*\n${json.deskripsi}`, author, null, [['⫷ ᴄᴀᴋ ʟᴏɴᴛᴏɴɢ ⫸', `${usedPrefix}caklontong`]], conn.caklontong[id][0])
