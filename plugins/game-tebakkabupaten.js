...
Bonus: ${poin} XP
  ...
  conn.tebakkabupaten[id] = [
    await conn.sendButtonImg(m.chat, await (await fetch(json.url)).buffer(), caption, '', '⫷ ʙᴀɴᴛᴜᴀɴ ⫸', '.tebu', m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakkabupaten[id]) await conn.sendBut(m.chat, `Waktu habis!\nJawabannya adalah *${json.title}*`, '', '⫷ ᴛᴇʙᴀᴋ ᴋᴀʙᴜᴘᴀᴛᴇɴ ⫸', '.tebakkabupaten', conn.tebakkabupaten[id][0])
